...
int sommeLigne(int Tab[T_4][T_4], int TabMask[T_4][T_4], int ligne, int col){
    //calcule la some d'une ligne d'une matrice 4x4
...
int verif_autour(int Tab[T_4])[T_4]{
    return 0;
}


//int CoupValide(int i,int j,int essai, int TabMask[T_4][T_4] ,int TabSolution[T_4][T_4]) {
//    // retourne 1 si lke coup est correct, 0 si valide mais inccorect, et si faux
    ...
    for (int i = 0; i < size; ++i) {
        for (int j = 0; j < size; ++j) {
            if (TabMask[i][j]!=1){
                ...
                // algo qui rempli au hasard
//                int essai = rand()%2;
//                if (CoupValide(i,j,0,Tab)==1)
//                    TabMask[i][j] = essai;
            }
        }
    }
...
int ComparerCases(int Tab[T_4][T_4],int Tab2[T_4][T_4],int ligne, int col){
    //retourne 1 si les 1 et 0 sont bien placés, 0 sinon (prends le masque en compte)
    if (Tab[i][j] != Tab2[i][j])
        return 0;
    return 1;
